...
export default class DestinationTemplate extends React.Component {
  constructor(props) {
    ...
    this.seo = {
      title: 'Destinations',
      description:
        'Read about the next big adventure on Where to Next, Doc? to help plan your next vacation.',
    }

    // Check if any data was returned for the query
    // May not be if there is no articles
    if (
      this.props.data.allMarkdownRemark === undefined ||
      this.props.data.allMarkdownRemark == null ||
      this.props.data.allMarkdownRemark.edges === undefined ||
      this.props.data.allMarkdownRemark.edges == null ||
      this.props.data.allMarkdownRemark.edges < 1
    ) {
      this.posts = null
      this.seo.image =
        this.props.location.origin +
        getSrc(this.props.data.file.childImageSharp)
      this.seo.keywords = [
        'Destinations',
        'North America',
        'South America',
      ...
      this.seo.image =
        this.props.location.origin +
        getSrc(this.posts[0].node.frontmatter.featuredImage.childImageSharp)
      this.seo.keywords = [
        'Destinations',
        'North America',
        'South America',
      ...
      this.seo.date = new Date(
        this.posts[0].node.frontmatter.date
      ).toISOString()
    }
  ...
  renderBlogPosts() {
    ...
    for (let i = 0; i < j; i += chunk) {
      ...
      postGroup = postGroup.map((item) => {
        ...
        return {
          childImageSharp: post.featuredImage.childImageSharp,
          alt: alt,
      ...
      finalPosts.push(<CardDeck key={i} items={postGroup} type={'vertical'} />)
    }

    return (
      <Row className="no-marg-pad justify-content-center">
        <Col md="11">{finalPosts}</Col>
      </Row>
    )
  }
          ...
          {this.posts && (
            <Row>
              <Col className="d-flex align-items-center justify-content-center">
                <PaginationNav
                  basePath={'/' + this.props.pageContext.basePath}
